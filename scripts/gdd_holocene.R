...
local_optima <- optimality_holocene %>%
  group_by(period, id) %>%
  dplyr::filter(opt == max(opt,na.rm = T)) 
...
optimum_plot <- ggplot() +
  ...
  theme(legend.position = 'none', panel.grid = element_blank(),
        strip.background = element_blank(), axis.title.x = element_text(size = 8)) +
  labs(y = "Optimality", x= "DOY") +
  coord_cartesian(xlim = c(0,365), 
                  ylim = c(min(global_optimum$opt), max(global_optimum$opt) +0.05), 
                  expand = FALSE)

ggsave2(filename = file.path(wd, "figures/supp", "optimality_holocene.pdf"),
        plot = optimum_plot, device = cairo_pdf)
