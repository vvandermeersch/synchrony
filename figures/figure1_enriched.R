...
optimum_plot_fut <- ggplot() +
  ...
  theme_bw() +
  labs(y = "Optimality", x= "DOY") +
  scale_y_continuous(position = 'right', breaks = seq(0.5,1.1,0.2)) +
  coord_cartesian(xlim = c(0,365), 
...
newfigure1 <- 
  pareto_front + 
  plot_spacer() +
  optimum_plot +   scale_x_continuous(position = "top") +
  optimum_plot_hol + 
  plot_spacer() +
  optimum_plot_fut + 
...
cowplot::ggsave2(filename = file.path(wd, "figures", "figure1_enriched.pdf"),
                 plot = newfigure1, device = cairo_pdf, width = 130, height = 105, unit = "mm")
