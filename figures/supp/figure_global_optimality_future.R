...
optimum_plot_fut <- ggplot() +
  ...
  coord_cartesian(xlim = c(0,365), 
                  ylim = c(min(global_optimum_fut$opt), 1.25), 
                  expand = FALSE) +
...
cowplot::ggsave2(filename = file.path(wd, "figures/supp", "optimality_future.pdf"),
                 plot = optimum_plot_fut, device = cairo_pdf, width = 105, height = 58, unit = "mm")
