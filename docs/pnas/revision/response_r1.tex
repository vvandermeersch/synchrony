...
We are happy to hear that by both reviewers recognized the value of our work and we appreciate the opportunity to revise our manuscript. We have worked to clarify the points raised by the editor and the two reviewers in this new version. \\
...
We apologize for the lack of clarity. We have revised the beginning of our introduction to clarify the transition between the general discussion of environmental cues and the specific focus on solstice, and to introduce the solstice hypothesis more clearly and explicitly.% Consider removing trigger from the paper? If you do I would mention it here, saying you followed language in previous studies, but now see it made the hypotheses and paper less clear, so now focus mainly on using 'cue' and explaining the hypotheses more clearly. 
...
\begin{quote}
    ``Summer solstice appears as a less reliable cue across space. We found substantial variation in the optimal timing when examined across Europe and North America [...]''
\end{quote}
...
We agree and have now  made the scope of the study clearer, by adding two inset plots for North America in Fig. 1 and Fig. 2 (previously \emph{data not shown}). We also clarify at the beginning of the Introduction that the summer solstice hypothesis has been proposed for temperate trees in the Northern Hemisphere (\href{file:synchrony_r1\#lintarget:temptree}{lines \ref*{lin:temptree}}, \href{file:synchrony_r1\#lintarget:north}{\ref*{lin:north}}, \href{file:synchrony_r1\#lintarget:temptree2}{\ref*{lin:temptree2}} \& \href{file:synchrony_r1\#lintarget:moist}{\ref*{lin:moist}}).\\
...
We thank the reviewer for their positive comments regarding our manuscript and appreciate their concerns regarding moisture. We detail our changes regarding this below.\\
...
We focused on thermal cues as a simple alternative to the solstice hypothesis, but we agree that this approach does not capture the full complexity of tree growth dynamics across all ecosystems. Both soil moisture and vapor pressure deficit strongly influence plant water status and can constrain growth, particularly in water-limited regions, leading to a summer interruption of growth (e.g. from mid-July to mid-September in southern France; \citealt{Lempereur2015}). We now acknowledge this limitation more explicitly throughout the manuscript (lines \href{file:synchrony_r1\#lintarget:moist}{\ref*{lin:moist}}, \href{file:synchrony_r1\#lintarget:moist2}{\ref*{lin:moist2}}, \href{file:synchrony_r1\#lintarget:moist3}{\ref*{lin:moist3}}, \href{file:synchrony_r1\#lintarget:moist4}{\ref*{lin:moist4}} \& \href{file:synchrony_r1\#lintarget:moist5}{\ref*{lin:moist5}}) and have incorporated the suggested reference \citep{Garonna2018}, as well as \citet{Zhao2010} and \citet{Seo2025}.\\
...
We agree that this added unnecessary complexity, and have removed the expression from the Introduction.\\
...
Thank you for this suggestion. We now explicitly indicate that this hypothesis was proposed for temperate tree species (\href{file:synchrony_r1\#lintarget:temptree}{line \ref*{lin:temptree}}).\\
...
Our apologies, as we can see now that this was not clear. We now introduce the switch in the second paragraph (\href{file:synchrony_r1\#lintarget:switch}{line \ref*{lin:switch}}):
\begin{quote}
    ``Recently, summer solstice has been proposed as a universal cue for temperate trees to switch their responses to temperature [...]''
\end{quote}
We also now mention the moisture constraints (\href{file:synchrony_r1\#lintarget:moist}{line \ref*{lin:moist}}):
\begin{quote}
    ``Further, as the growing season is likely to be increasingly constrained by moisture availability in many northern temperate latitudes with climate change \citep{Garonna2018} [...]''
\end{quote}
...
\begin{quote}
    ``alongside other constraints, such as chilling requirements \citep{Chuine2017} and moisture availability \citep{Garonna2018}''
\end{quote}
...
\begin{quote}
    ``We did not consider moisture availability constraints along the growing season, that will likely increase with climate change \citep{Garonna2018, Zhao2010, Seo2025}.''
\end{quote}
...
We are not sure of the reviewer's concerns regarding this sentence, but we have adjusted the next paragraph to stress the relevance of soil moisture as well (\href{file:synchrony_r1\#lintarget:moist5}{line \ref*{lin:moist5}}). \\
...
We are glad that the reviewer found our study very interesting and well-written. We thank them for the feedback, which has improved the manuscript.\\
...
Thank you for this suggestion, which was also raised by Reviewer 1. We now clarify early in the Introduction that we focus on temperate latitudes in the Northern Hemisphere (\href{file:synchrony_r1\#lintarget:temptree}{lines \ref*{lin:temptree}}, \href{file:synchrony_r1\#lintarget:north}{\ref*{lin:north}} \& \href{file:synchrony_r1\#lintarget:temptree2}{\ref*{lin:temptree2}}). \\ 

\begin{mybox}
\emph{L17 - leaf unfolding of what?}
\end{mybox}

Apologies for the lack of clarity, we have added: ``of temperate deciduous trees'' (\href{file:synchrony_r1\#lintarget:temptree2}{line \ref*{lin:temptree2}}).\\
...
Apologies as we should have included this. We now explicitly mention the temperature range in the text (\href{file:synchrony_r1\#lintarget:range}{line \ref*{lin:range}}).\\
We found very similar results when using a temperature range of 0\degree C to 40\degree C:
\begin{figure}[h]
	\centering
	\vspace*{-0.2cm}
...
Additionally, we have posted all our code (\url{10.5281/zenodo.15186138}) and links to the public data so that others can remake this figure with varying GDD thresholds. \\
...
Many thanks for the positive feedback on this figure and our apologies for failing to fully explain it. The dashed line indicates the summer solstice (DOY = 172), and we have added this information to the figure caption.\\

\begin{mybox}
\emph{L69 consider replacing 'only' with 'on' or some other rewording of this sentence to make more sense.}
\end{mybox}

We have added ``on'' and thank the reviewer for this fix.\\
