...
% But which cue plants need to track to understand that it is the optimal period? Temperature (and daily GDD accumulation) peaks after the optimum, so it should be something else?
...
% \item This means:
% difficult to isolate the solstice
% at the solstice, plants have often already accumulated substantial GDD
% \begin{enumerate}
%     \item critical need for experiments to disentangle daylength vs temp.
% TO READ: https://doi.org/10.1111/1365-2435.14329
% controlled environments:
% well designed experiments that control for covariation among temperature and light treatments
...
% \end{enumerate}
% local-scale trends across climatic gradients to see how plant responses to solstice and temperature vary regionally
% h Once local-scale trends are understood, how local trends scale up to broader (subcontinental) scales, and influence synchrony patterns
% =>  inform phenological models, allowing researchers to build more accurate predictions by incorporating the specific cues plants use to trigger growth
%\end{enumerate}
